...
/**
 * FluidSim — the classic PavelDoGreat WebGL Fluid Simulation, wrapped as a
 * React component. Colorful dye splats follow the pointer over a black
 * background. Colors are set in HSVtoRGB (brand cyan / purple / pink).
 *
 * Renders a fixed full-screen canvas behind everything (zIndex -1).
...
export default function FluidSim() {
  ...
  useEffect(() => {
    ...
    // Brand palette: cyan / purple / pink
    // Blue-first behavior: the first few color picks are forced to blue,
    // then it opens up to the full cyan/purple/pink cycle.
    let colorPickCount = 0
    const BLUE_FIRST_COUNT = 8 // how many initial picks are blue

    function generateColor() {
      let c
      if (colorPickCount < BLUE_FIRST_COUNT) {
        c = { r: 60, g: 184, b: 211 } // brand cyan/blue
      } else {
